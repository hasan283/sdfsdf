var eyeAnimation = (function () {
    function eyeAnimation(container) {
        // Conf
        this.eyeWidth = eyeAnimation.conf.eyeWidth;
        this.eyeHeight = eyeAnimation.conf.eyeHeight;
        this.origins = eyeAnimation.conf.origins;
        this.radius = eyeAnimation.conf.radius;

        // Container dimensions
        this.container = container;
        this.container.style.width = this.eyeWidth + 'px';
        this.container.style.height = this.eyeHeight + 'px';
    ...
    eyeAnimation.init = function () {
        // Find existing eyeAnimations
        var i, containers = [];
        if ('getElementsByClassName' in document) {
            containers = document.getElementsByClassName('eyeAnimation');
        } else {
            var elements = document.getElementsByTagName('*');
            for (i = 0; i < elements.length; i++) {
                if (elements[i].className.match(/\beyeAnimation\b/, 'g')) {
                    containers.push(elements[i]);
                }
            }
        }
        for (i = 0; i < containers.length; i++) {
            new eyeAnimation(containers[i]);
        }

        // Listen for mouse movement
        document.addEventListener('mousemove', function (e) {
            eyeAnimation.lastMouseCoords = {
                x: e.clientX,
                y: e.clientY
            };
            eyeAnimation.updatePupils();
        }, false);
    };
    eyeAnimation.lastMouseCoords = null;
    eyeAnimation.updatePupils = function () {
        if (eyeAnimation.lastMouseCoords) {
            for (var i = 0; i < eyeAnimation.balls.length; i++) {
                eyeAnimation.balls[i].drawPupilsFromMousePosition(eyeAnimation.lastMouseCoords);
            }
        }
    };
    eyeAnimation.prototype = {
        setupCanvas: function (canvas) {
            // Append canvas and setup the 2d drawing context
            canvas.width = this.eyeWidth;
            canvas.height = this.eyeHeight;
        ...
        clearCanvas: function () {
            this.clear(0, 0, this.eyeWidth, this.eyeHeight);
        },
        drawEyeAnimation: function (x, y) {
        ...
        drawPupils: function (coords) {
            ...
            this.clearCanvas();
            this.drawEyeAnimation(this.origins.x1, this.origins.y1);
            this.drawEyeAnimation(this.origins.x2, this.origins.y2);
        ...
        getPupilPositionsFromMousePosition: function (x, y) {
            // Normalize x and y to container page offset
